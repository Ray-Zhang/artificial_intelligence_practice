...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minimax(self, gameState, agentIndex, step):
            if gameState.isWin() or gameState.isLose():
                ...
                if agentIndex == 0:
                    return max([minimax(self, gameState.generateSuccessor(agentIndex, action), (step+1)%numAgents-1, step+1) for action in gameState.getLegalActions(agentIndex)])
                else:
                    return min([minimax(self, gameState.generateSuccessor(agentIndex, action), (step+1)%numAgents-1, step+1) for action in gameState.getLegalActions(agentIndex)])

	maxEvaluate = -999999
	for action in gameState.getLegalActions(0):
	    evaluate = minimax(self, gameState.generateSuccessor(0, action), 1, 2)
